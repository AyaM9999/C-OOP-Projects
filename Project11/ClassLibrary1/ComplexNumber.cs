...
namespace ClassLibrary1
{
    public class ComplexNumber
    {
        ...
        public static ComplexNumber operator *(ComplexNumber c1, ComplexNumber c2)
        {
            double real= (c1.realPart*c2.realPart)-(c1.imgPart*c2.imgPart);
            double img=(c1.realPart*c2.imgPart)+(c1.imgPart*c1.realPart);
            ComplexNumber result = new ComplexNumber((int)real, (int)img);
            return result;
        }
        ...
        {
            double real, img;
            double denominator = Math.Pow(c2.realPart,2) + Math.Pow(c2.imgPart,2);
            if (denominator != 0)
            {
                real = ((c1.realPart * c2.realPart) + (c1.imgPart * c2.imgPart)) / denominator;
                 img = ((c1.realPart * c2.imgPart) + (c1.imgPart * c1.realPart)) / denominator;
                ComplexNumber result = new ComplexNumber((int)real, (int)img);
                return result;
            }
            else
